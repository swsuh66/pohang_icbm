...
<head>
    ...
    <%@include file="/resources/inc/ol.inc" %>
        ...
        * 미터기 상태 차트 갱신
        ...
        function refreshStatChart(result) {
            ...
            if (chked) {
                ...
                tData = [0, 0, 0, 0, 0, 0, 0];
                labels = ['정상', '통신장애', '계량기 장애', 'Q4 초과', '역류', '누수', '배터리 장애'];
            } else {
                dataOrder[-1] = {order: 0}; //정상
                dataOrder[0] = {order: 1}; //통신 장애
                dataOrder[1] = {order: 2}; //계량기 장애
...
<div id="mapContainer" class="map-container">
    ...
    <div class="box-list-wrap right">
        <div class="search-group">
            <input type="text" placeholder="특정 수용가 데이터 검색" id="searchInputP"
                   onKeypress="javascript:if(event.keyCode == 13) { parent.loadPointSearch( makeParams() ); parent.loadData();}">
            <button type="button" onclick="parent.loadPointSearch( makeParams() ); parent.loadData();"><i class="ico i-primary-search"></i>
            </button>
        </div>
