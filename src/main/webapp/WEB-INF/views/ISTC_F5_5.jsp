...
<script type="text/javascript">
	...
	var dbParamsTb = 'f5-custExport';
	
	var dbImportTb = 'f5-custImport';
	...
	* 그리드 컬럼 요소 리빌딩
	...
	var mainFields = [
     	{ name: "num", 			title: "순번", 	     type: "text", align:"center", width: 60, 	itemTemplate:colfnc, sortingDisabled:true,hasGroup:true},
     	{ name: "adminNo",     title: "고객번호", 	 type: "text",   width: 140,  itemTemplate:colfnc, hasGroup:true},
        { name: "tapGb",     title: "수전구분", 	 type: "text",   width: 140,  itemTemplate:colfnc, hasGroup:true},
        { name: "tapNo",     title: "수전번호", 	 type: "text",   width: 140,  itemTemplate:colfnc, hasGroup:true},       
        { name: "udtDt",     title: "수정일", 	 type: "text",   width: 100,  itemTemplate:colfnc, hasGroup:true},
	    {       		
	        itemTemplate: function(_, item) {
	        

	        	var iEl = $("<i>")
	        		      .attr("data-adminNo", item.adminNo)
	        			  .attr("data-tapGb", item.tapGb)
	        			  .attr("data-tapNo", item.tapNo)
	        			 .append("작업취소")
	        	...
	        	var aEl = $("<a>").attr("href", "#none")
	        			  ...
	        			  .on("click", function (evt) {
	        				  ...
	        				  var el = $(evt.target);
		                      var adminNo = el.data('adminNo');
		                      var tapGb = el.data('tapGb');
		                      var tapNo = el.data('tapNo');

		                      var params = new Object();
		                      params.adminNo = item.adminNo;
		                      params.tapGb = item.tapGb;
		                      params.tapNo = item.tapNo;
		                      
		                      
	        				  
		                      jAlert.confirm('알림','작업을 취소하시겠습니까?', function(){
		                    	  
		                    	  updateAjax('mars.icbm.map1.updateCustomer', params, function() {
	...
	* 그리드 초기화
	...
	function loadData() {
		...
		/* 수용가 조회 */
		getAjax('custList_paging', params, function() {
	...
	function exFileDownload() {
		...
		window.location = getContextPath() + '/resources/excel/custImport.xlsx';
...
<div role="main" class="sub-content">
	...
	<div class="sub-cont-header">
		...
		<div class="sub-cont-header-area">
			<div class="dj-input-group search">
				<div class="dj-search-group">
					<input type="text" id="searchInputCtn" placeholder="고객번호" onKeypress="javascript:if(event.keyCode == 13) { mainGrid.search(); }">
				</div>
			</div>
			<c:if test="${user.getSiteLv() == 1}">
				<button type="button" onclick="openModal();" class="btn btn-sm dj-btn-outline-primary" data-toggle="dropdown" data-display="static" aria-haspopup="true" aria-expanded="false"><i
						class="ico i-import"></i>Import
				</button>
			</c:if>
		</div>
	</div>
              ...
              <div class="ml-auto d-flex">
                ...
                <div class="d-flex align-items-center px-lg-0">               
                  ...
                  <div class="d-flex align-items-center mx-4 mx-lg-0">
	                ...
	                <input type="text" id="searchInputCtn" placeholder="고객번호" class="pl-45 text-black form-control form-control-lg bgc-transparent brc-yellow-tp3 brc-on-focus border-none border-b-1 radius-0 shadow-none" onKeypress="javascript:if(event.keyCode == 13) { mainGrid.search(); }">			                
	                <a href="#none" title="검색 문구 지우기" class="btn btn-outline-warning btn-brc-tp radius-3px py-2" onclick="$('#searchInputCtn').val('')"> <i class="fa fa-eraser text-140"></i></a>
