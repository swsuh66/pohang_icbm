...
function dayStockchart(container, data) {
	...
	var clr = [Highcharts.getOptions().colors[7], Highcharts.getOptions().colors[2], Highcharts.getOptions().colors[3], Highcharts.getOptions().colors[5]];
	for (var ix = 0; ix < clr.length; ix++) clr[ix] = new Highcharts.Color(clr[ix]).setOpacity(1).get();
	...
	this.initChart = function (container) {
		...
		if (data) this.setDataSource(data);
	};
...
function rawStockchart(container, data) {
	...
	this.setDataSourceNew = function (data, beginDate, endDate) {
		...
		if (data) {
			for (var ix = 0; ix < data.length; ix++) {
				const dateStr = data[ix].measDtStr;
				const [year, month, day, hour, min, sec] = dateStr.split('-').map(Number);
				const ts = Date.UTC(year, month - 1, day, hour, min, sec);
				dataMap[ts] = data[ix]; // key: timestamp
			}
		}

		// 1시간 단위 간격으로 X축을 생성 (또는 필요시 하루 단위)
		var interval = 60 * 60 * 1000; // 1시간
		for (let ts = beginTime; ts <= endTime; ts += interval) {
	...
	var clr = [Highcharts.getOptions().colors[0], Highcharts.getOptions().colors[1], Highcharts.getOptions().colors[2], Highcharts.getOptions().colors[3]];
	for (var ix = 0; ix < clr.length; ix++) clr[ix] = new Highcharts.Color(clr[ix]).setOpacity(1).get();
