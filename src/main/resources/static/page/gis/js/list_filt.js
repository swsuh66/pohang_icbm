...
function filtListData() {
	...
	if (value) {
		$('#filt-text').blur();
		if (value.substr(0, 1) == '.') data = do_filtListData(data, 'pointSq', parseInt(value.substr(1)));
		else if (value.substr(0, 1) == '#') data = do_filtListData(data, 'amiSn', value.substr(1));
		else if (value.substr(0, 1) == '@') data = do_filtListData(data, 'meterId', value.substr(1));
		else data = do_filtListData(data, 'custNm', value);
	...
	if (value) {
		if (value.substr(0, 4) == 'blk_') {
			...
			if (value != '0') $('#filter-select').prop('selectedIndex', 0);
...
function updateFiltItems(blkData) {
	...
	if (blkData && blkData.length) {
		...
		for (var ix = 0; ix < blkData.length; ix++) {
			...
			if (item.level == 0) rst += '<optgroup label="' + prefix + label + '">';
			else {
	...
	rst += '<option value="stt_01000000" class="select_opt">Q3 초과</option>';
